...
namespace LinqProject
{
    internal class Program
    {
        ...
        {
            // single line query
            var result = products.Where(p => p.ProductName.Contains("top")).OrderByDescending(p => p.UnitPrice).ThenByDescending(p => p.UnitsInStock);
            foreach (var product in result)
            {
                Console.WriteLine(product.ProductName);
            }
        ...
        private static void Test(List<Product> products)
        {
            ...
            List<Product> filteredP = GetProductsLinq(products);
            foreach (var p in filteredP)
            {
